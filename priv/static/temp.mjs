...
var Spa = class _Spa {
  ...
  #runtime;
  constructor(root3, [init, effects], update3, view2) {
    this.#runtime = new Runtime(root3, [init, effects], view2, update3);
  }
  send(message) {
...
function simple(init, update3, view2) {
...
var Attempt = class extends CustomType {
...
var HandleKeyPressEvent = class extends CustomType {
  constructor(key) {
    super();
    this.key = key;
  }
...
function handle_submit_wordle(model) {
  ...
  if (!is_full_word) {
    ...
    let progress = _block;
    let _block$1;
    let _pipe$1 = model.attempts;
    _block$1 = append(
      _pipe$1,
      (() => {
        let _pipe$2 = new Attempt(model.guess, progress);
        return wrap(_pipe$2);
      })()
    );
    let attempts = _block$1;
    echo(attempts, "src/msg.gleam", 49);
    let _record = model;
    return new Wordle(
      _record.level,
      _record.solution,
...
function handle_key(model, key) {
  ...
  if (lowercased_key === "enter") {
    ...
    if (!$) {
      let _pipe$1 = model;
      let _pipe$2 = handle_submit_wordle(_pipe$1);
      let _pipe$3 = clear_guess_input(_pipe$2);
      return handle_correct_guess(_pipe$3);
    } else {
      let _pipe$1 = model;
      return handle_next_level(_pipe$1);
    }
  } else if (lowercased_key === "backspace") {
    let _block$1;
    let _pipe$1 = model.guess;
    _block$1 = remove_letter(_pipe$1);
    let new_guess = _block$1;
    let _record = model;
    return new Wordle(
      _record.level,
      _record.solution,
    ...
    if (!$) {
      ...
      let _block$1;
      let _pipe$1 = model.guess;
      _block$1 = add_letter(_pipe$1, letter);
      let new_guess = _block$1;
      let _record = model;
      return new Wordle(
        _record.level,
...
function update2(model, msg) {
  ...
  } else if (msg instanceof PlayerNextLevel) {
    let _pipe = model;
    return handle_next_level(_pipe);
  } else {
    let key = msg.key;
    let _pipe = model;
    return handle_key(_pipe, key);
  }
}
...
function row_1_view() {
  ...
  return ((_capture) => {
    return div(
      toList([
        class$("flex justify-center w-full gap-x-1 sm:gap-x-1.5")
      ]),
      _capture
    );
  })(_pipe$2);
...
function progress_to_color(progress) {
  if (progress instanceof NotSet) {
    return "bg-white";
  } else if (progress instanceof Absent) {
    return "bg-gray-300";
  } else if (progress instanceof Present) {
...
function tiles_view(guess, progress) {
  let _block;
  let _pipe = progress;
  _block = map(_pipe, progress_to_color);
  let progress_colors = _block;
  return div(
    toList([class$("flex grid grid-cols-5 gap-1")]),
    (() => {
      let _pipe$1 = guess;
  ...
  return div(
    ...
    (() => {
      ...
      return map(
        _pipe,
        (attempt) => {
          return tiles_view(attempt.word, attempt.progress);
        }
      );
    })()
  );
...
function main() {
  ...
  return on_keypress(
    (key) => {
      let _pipe = key;
      let _pipe$1 = new HandleKeyPressEvent(_pipe);
